...
const char *WINDOWTITLE = { "OpenGL / GLUT Project5 -- Yu Zhang" };
...
Display()
{
	...
	T0 = .5;
	float Color[3] = { 1.,1.,0. };
	float ps = (.5 + .5*sin(2.*M_PI*Time));
	float pt = .1*(.5 + .5*sin(2.*M_PI*Time));
	Pattern->Use();
	//Pattern->GetUniformLocation("ourcolor");
	//Pattern->SetUniformVariable("ourColor", Color);
	//Pattern->SetUniformVariable("uTexUnit", sampler);
	Pattern->SetUniformVariable("Ds", ps);
	Pattern->SetUniformVariable("Dt", pt);
