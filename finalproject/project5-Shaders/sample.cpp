...
const char *WINDOWTITLE = { "OpenGL / GLUT Final Porject -- Yu Zhang" };
...
GLfloat vertices[] = { -.5f, -.5f,.0f,
.5f, -.5f, .0f,
.0f, .5f, .0f };
GLfloat colors[] = { 1.0f, .0f, .0f, 1.0f,
.0f, 1.0f, .0f, 1.0f,
.0f, .0f, 1.0f, 1.0f };
...
const GLfloat BACKCOLOR[ ] = {138./255., 235./255., 255./255., 1. };
...
//	if (bvhList.GetSize())
//	{
//		for (int i = 0; i < bvhList.GetSize(); i++)
//		{
//			bvhList.SetAnimationTime(i, bvhList.GetAnimationTime(i) + delta);
//		}
//	}
//}
...
//		{
//			for (int i = 0; i < bvhList.GetSize(); i++) {
//				bvhList.SetAnimationTime(i, bvhList.GetAnimationTime(i) + 0.03f);
//			}
//		}
//#endif
		if (bvhList.GetSize())
		{
			...
			for (int i = 0; i < bvhList.GetSize(); i++) {
				bvhList.SetFrameNo(i, bvhList.GetAnimationTime(i) / bvhList.GetBVHIndex(i)->GetInterval());
				bvhList.SetFrameNo(i, bvhList.GetFrameNo(i) % bvhList.GetBVHIndex(i)->GetNumFrame());
			}
		}
...
Display( )
{
	...
	SetSpotLight(GL_LIGHT0, 0., 2., 0., 0., -1., 0., 1., 1., 1.);
	SetSpotLight(GL_LIGHT2, 0., 3, -3., 0., 0., -1., 1., 1., 1.);
	SetPointLight(GL_LIGHT1, -2., 1.5, -5., 0., 0., 1.);
	...
	//glScalef(1.5, 1., 1.5);
	//glRotatef(90., 0., 1., 0.);
	glBindTexture(GL_TEXTURE_2D, WALLOUT);
	...
	if (bvhList.GetSize())
	{
		for (int i = 0; i < bvhList.GetSize(); i++)
		{
			bvhList.GetBVHIndex(i)->RenderFigure(bvhList.GetFrameNo(i), 0.02f);
		}
	}
...
InitMenus( )
{
	...
	glutAddMenuEntry("Select BVH File", BVHFILE);
...
InitGraphics( )
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, 3, Width1, Height1, 0, GL_RGB, GL_UNSIGNED_BYTE, Texture);

	//texture the wall out
	Texture2 = BmpToTexture("wallout.bmp", &Width2, &Height2);
	//glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glGenTextures(1, &WALLOUT);
	glBindTexture(GL_TEXTURE_2D, WALLOUT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	...
	//teture the people's picture
	Texture3 = BmpToTexture("people.bmp", &Width3, &Height3);
	glGenTextures(1, &PEOPLE);
	glBindTexture(GL_TEXTURE_2D, PEOPLE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	...
	//bool valid = Pattern->Create("verLight.vert", "fragLight.frag");
	////bool valid = Pattern->Create("vsShaders.vert", "fragShaders.frag");

	//Pattern->SetVerbose(true);
	//Pattern->
	//if (!valid) {
	//	//GLuint vertexID = 
	//}
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
		...
		case 'D':
			if (bvhList.GetSize())
			{
				bvhList.RemoveBVHIndex(bvhList.GetSize() - 1);
			}
			break;
		...
		case 'L':
			...
			{   
				...
				{
					//delete bvh;   
					delete bvhList.GetBVHIndex(bvhList.GetSize() - 1);
				}
			}
#endif
			break;
...
Reset( )
{
	...
	if (bvhList.GetSize())
	{
		for (int i = bvhList.GetSize(); i > 0; i--) {
			bvhList.RemoveBVHIndex(bvhList.GetSize() - 1);
		}
	}
}
...
SetPointLight(int ilight, float x, float y, float z, float r, float g, float b)
{
	...
	glLightf(ilight, GL_CONSTANT_ATTENUATION, 2.);
